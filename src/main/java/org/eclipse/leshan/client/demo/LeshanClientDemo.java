...
public class LeshanClientDemo {
    ...
    public static void main(final String[] args) {
        ...
        options.addOption("queue", false, "If present then enable queue mode");
        ...
        try {
            createAndStartClient(endpoint, localAddress, localPort, cl.hasOption("b"), additionalAttributes,
                    ...
                    cl.hasOption("f"), modelsFolderPath,enableQueueMode);
        } catch (Exception e) {
    ...
    public static void createAndStartClient(String endpoint, String localAddress, int localPort, boolean needBootstrap,
                                            ...
                                            boolean forceFullhandshake, String modelsFolderPath, final boolean enableQueueMode) throws CertificateEncodingException {
        ...
        if (needBootstrap) {
            ...
            if (pskIdentity != null) {
                ...
                initializer.setInstancesForObject(SERVER, getServer(lifetime, enableQueueMode));
            } else if (clientPublicKey != null) {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread() {
            @Override
            public void run() {
                LOG.info("**destroying the client with option {}",!enableQueueMode);
                client.destroy(!enableQueueMode); // send de-registration request before destroy
            }
        });
        ...
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            while (scanner.hasNext()) {
                ...
                if (command.startsWith("create")) {
                    ...
                    LOG.info("stopping the client without deregister");
                    client.stop(false);
    ...
    private static Server getServer(int lifetime, boolean isQueue) {
        BindingMode bindingMode = isQueue ? BindingMode.UQ: BindingMode.U;
        LOG.info("Initiliazing the server object with id {} lifetime {} bindingMode {}",123,lifetime,bindingMode);
        return new Server(123, lifetime, bindingMode, false);
    }
}
